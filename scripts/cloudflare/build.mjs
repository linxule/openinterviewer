...
//
// Usage: node scripts/cloudflare/build.mjs [--config wrangler.jsonc]
//
// Every build runs the Next build: the manifest records the current commit,
// so packaging a .next left over from another commit would mislabel it.
// Unknown options are refused rather than ignored.
...
export function parseBuildArgs(argv) {
  const { values } = parseArgs({
    args: argv,
    options: { config: { type: 'string', default: 'wrangler.jsonc' } },
    strict: true,
    allowPositionals: false,
  });
  return { configArg: values.config };
}

/** The OpenNext invocation: always a full Next build. */
export function openNextBuildArgs(configPath) {
...
async function main(argv) {
  ...
  await run(binPath('opennextjs-cloudflare'), openNextBuildArgs(configPath), { env: buildEnv });
  ...
  const schemaModule = await import(pathToFileURL(path.join(ROOT, 'cloudflare', 'workspace', 'schema.ts')).href);
  const git = gitState();
