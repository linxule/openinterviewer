...
The JSON on stdin holds only `{"ADMIN_PASSWORD": "…", "OPENAI_API_KEY": "…"}`. Never keep it in a file inside the checkout, where `git add` can pick it up: pipe it from a secret manager as above (the template holds references such as `{{ op://<vault>/<item>/password }}`, not values, and is kept outside the checkout like any other file you add), or drop `--secrets-stdin` and type the values at the installer's hidden prompt. If you must use a file, keep it outside the repository and delete it afterwards. The installer generates the session, participant, rate-limit and operator secrets and the recovery epoch, and sends all secrets to Cloudflare through stdin. It records a non-secret receipt in `cloudflare/installations/`. Updates use `setup:cloudflare -- update`; interrupted installs use `resume`. The project's own production installation is instead deployed by the manually dispatched `promote-cloudflare` job in `.github/workflows/ci.yml`, which runs the full release check first; see [Maintained instance: CI promotion](docs/operations/cloudflare-migration/INSTALLER.md#maintained-instance-ci-promotion). See [INSTALLER.md](docs/operations/cloudflare-migration/INSTALLER.md) and the operator [RUNBOOK.md](docs/operations/cloudflare-migration/RUNBOOK.md) for maintenance modes, operational backup/import, restore and rollback. A coding agent can drive the same commands with [`skills/openinterviewer-cloudflare`](skills/openinterviewer-cloudflare/SKILL.md).
