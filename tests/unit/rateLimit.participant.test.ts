...
describe('participant AI rate limiting', () => {
  ...
  // LIMITS.synthesis and the participant refund helper are deleted (slice P
  // §P3.3): the participant path no longer calls the provider at all, and a
  // refund helper with no caller is a loaded gun for the next call site that
  // reaches for the nearest-looking budget.
});
