...
- Standalone researcher sign-in (both targets): `src/app/api/auth/route.ts` bounds the body to 1 KiB and admits each attempt against a failed-attempt budget before comparing the password, failing closed (503) when the budget is unavailable. The client subject (IPv4 address, IPv6 /64, or the shared `unknown`/`subrequest` scope) is `signInBudgetSubject` in `src/lib/runtime/clientAddress.ts`; the budget is `cloudflare/workspace/login.ts` on Cloudflare and `src/lib/storage/redisLoginBudget.ts` on Node
