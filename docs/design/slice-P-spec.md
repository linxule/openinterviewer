...
`feat/synthesis-uses-study-model` has landed on `main` at `f6bb49c`: `resolveSynthesisModel(studyConfig)` returns `studyConfig.aiModel` and throws when the study names none (`src/lib/providers/synthesisModel.ts:23-30`). Every "the analysis model" below means the study's configured model **at the moment the analysis ran**.
...
| `saved` | `saveStatus === 'saved'` | Ruling 4: the heading becomes `Thank you`; the safe-to-close sentence stays verbatim; then P12's thank-you sheet and the K2 receipt |
...
- The `!isAdmin` consent block (`:94-128`) and its `participantRateLimitResponse` call go with it, and `LIMITS.synthesis` (`rateLimit.ts:40-48`) loses its only consumer and is **deleted**.
- `refundParticipantRateLimit` (`rateLimit.ts:199-214`) loses its only call site and is **deleted**, with `REFUND_LIMITS_SCRIPT` (`:82-89`), the `rateLimit.refund` entry in `REQUEST_LOG_EVENT_ALLOWLIST` (`requestLog.ts:28`), and the describe block at `tests/unit/rateLimit.participant.test.ts:94-145`.
...
export interface InterviewAnalysisState {
  ...
  claimId?: string;          // present only while `running`; the CAS token (P6)
  claimedAt?: number;
...
export interface StoredInterview {
   ...
   * ran under. That divergence is why slice O's `conductedBy*` still exists.
   */
  aiProvider?: AIProviderType;
  aiModel?: string;
  requestedAiModel?: string;
  routedProvider?: string;
  conductedByProvider?: AIProviderType;  // slice O, unchanged
  conductedByModel?: string;             // slice O, unchanged
...
In `interviewSubmission.ts`, `ValidInterviewSubmission.synthesis` becomes `SynthesisResult | null` and `_receipt` leaves the type; `:133-137` becomes "absent or null → `null`, otherwise `validateSynthesisResult(input.synthesis)`" with **no** `_receipt` requirement. The explicit seven-field return copy at `:139-147` is unchanged in shape — it remains a whitelist, and P16 pins that with an adversarial regression rather than trusting it to stay one.
...
```ts
    const fingerprint = submissionFingerprint({
      id: interviewId, studyId: canonical.study.id,
      participantProfile: clientData.participantProfile, transcript: clientData.transcript,
      behaviorData: clientData.behaviorData, createdAt: clientData.createdAt ?? null,
      consentHash: consentRecord!.consentHash, consentAcceptedAt: consentRecord!.acceptedAt,
      conductedByProvider: canonical.study.config.aiProvider,
      conductedByModel: canonical.study.config.aiModel,
    });
```
...
`{ success, id, created, duplicate? }` (`:303-308`), unchanged. It says nothing about the analysis: the participant's client has no use for the fact, and the response must never become a place where an analysis outcome is awaited.
...
```
KEYS[1] interview:<id>
ARGV[1] op 'claim'|'complete'|'fail'   ARGV[2] interviewId   ARGV[3] nowMs   ARGV[4] leaseMs
...
```ts
  analysis: {
    // arity 1: unavailable, notfound, busy, done, stale, written, recorded
    'oi:analysis-unavailable': { arity: 1 }, /* …six more at arity 1… */
    'oi:analysis-claimed': { arity: 2, payloadKind: 'string' },  // the claimId
  },
```
...
export async function claimInterviewAnalysis(
  interviewId: string, client?: RedisPort, nowMs?: number): Promise<ClaimAnalysisResult>;

export async function attachInterviewAnalysis(input: {
  interviewId: string; claimId: string; synthesis: SynthesisResult;
  provenance: SynthesisProvenance; studyRevision: number;
...
export async function recordInterviewAnalysisFailure(
  ...
  failureKind: InterviewAnalysisFailureKind, client?: RedisPort): Promise<AttachAnalysisResult>;
```
...
// after the success response is constructed, before returning it:
    if (persistence.status === 'created') {
      const kvClient = context.kvClient, study = canonical.study;
      const providerKeys = providerKeysFromContext(context);
      after(() => runInterviewAnalysis({
        interviewId: interview.id, study, kvClient, providerKeys,
...
**(b) a fire-and-forget request from the participant's page** reintroduces the exact dependency the slice removes: a participant who closes the tab the instant they read "safe to close" — which the copy explicitly invites — cancels the analysis. It also needs a participant-authenticated route that performs a provider call, with its own abuse budget, consent re-verification and retry semantics, to do work the server already held every input for.

**(c) researcher-triggered only** is correct, and shipped — but as the *only* path it makes the common case (nothing failed) a manual press per interview. The owner's complaint is about who does the retrying, not who does the first attempt.

`after()` keeps the participant's response at today's latency minus a provider call, runs in the process that already holds the canonical study, the resolved provider keys and a live Redis client, and adds no authenticated surface.
...
- `studyId` comes from the query string, validated with the route's existing `STUDY_ID_PATTERN` (`interviews/[id]/route.ts:17`) and **required in both modes**, unlike the GET. Authority is per-study; an analyze without one has nothing to gate on.
- `getAuthorizedResearcherStudyContext(studyId, 'read')` then `configurationRequiredResponse`, byte-identical to `aggregate/route.ts:43-56` (P2.14 is the precedent for a researcher-authenticated write under `'read'`).
- `getInterviewChecked` + `mapInterviewLoad`, then **`if (mapped.interview.studyId !== studyId) return 404`** — the same cross-tenant refusal as `interviews/[id]/route.ts:58-60`. This is the line P16 attacks directly.
- `loadCanonicalStudy` for the config and revision, then `hostedAiRateLimitResponse(request, 'analysis', { researcherId })`, then `await runInterviewAnalysis(...)` — the researcher waits for this one, because they pressed a button.
- Response: `{ status: 'complete' | 'failed' | 'busy' | 'already-complete', failureKind? }`, 200 for all four. A failed analysis is a successful report of a failure, and the UI needs the kind to pick its copy. `providerErrorResponse` is not used: the researcher is being told a record fact, not a provider fact.
...
```ts
  analysis: { session: { maximum: 100, windowSeconds: 3_600 },
              network: { maximum: 200, windowSeconds: 3_600 },
              researcher: { maximum: 500, windowSeconds: 86_400 } },
```

`session` at 100/hour admits a 25-interview batch four times an hour; `researcher` at 500/day bounds platform exposure. The `after()` path keeps consuming `'synthesis'` with the participant session, exactly as the participant-triggered call does today — one save, one analysis, inside the existing budget.
...
- **`src/lib/synthesisReceipt.ts`**: `RECEIPT_VERSION`, `AUDIENCE`, `ISSUER`, `canonicalize`, `digest`, `createSynthesisReceipt` (`:36-72`), `verifySynthesisReceipt` (`:74-138`) and its v2 compatibility branch (`:117-127`), plus the `jose` import. `jose` stays in `package.json` — `auth.ts` still uses it.
- **`src/app/api/synthesis/route.ts`**: the `createSynthesisReceipt` call (`:157-169`) and `_receipt` in the response (`:185`). Preview returns the bare synthesis.
- **`src/lib/interviewSubmission.ts`**: the `_receipt` requirement (`:133-135`) and both `& { _receipt: string }` intersections (`:121`, `:136`).
- **`src/app/api/interviews/save/route.ts`**: the import, the call, the 403, and the `const { _receipt, ...verifiedSynthesis }` destructure (`:138`).

**What survives is the whole remaining file:** `SynthesisProvenance`, `validBoundedText`, `validateProvenance` and `aggregateProvenance` — called by `synthesis/aggregate/route.ts:162`, `generate-followup/route.ts:72`, and now `runInterviewAnalysis`. The file keeps its name with a rewritten header comment; renaming it to `provenance.ts` in the diff that deletes a signing path is two reviews wearing one hat (Deferred).
...
export async function runInterviewAnalysis(input: {
  interviewId: string; study: StoredStudy; kvClient: RedisPort; providerKeys: ProviderKeys;
...
1. `claimInterviewAnalysis`. Anything but `claimed` returns immediately, with no provider call. **This is the concurrency gate and it is first.**
2. `getInterviewChecked` for the transcript, profile and behaviour; a record that vanished between claim and read is `not-found`.
3. `getInterviewProvider(study.config, providerKeys)` → `provider.synthesizeInterview(...)`, same four arguments `synthesis/route.ts:151-156` passes.
4. `validateProvenance` on `result.execution` — a result not naming the provider and model that actually ran is `invalid-output` (AGENTS.md).
5. `attachInterviewAnalysis`. `too-large`/`unavailable` map to `failed` with `'too-large'`/`'storage'`; `stale` returns `busy` with no second write.
6. Any throw from step 3 → `recordInterviewAnalysisFailure(..., 'provider')`; a validation throw → `'invalid-output'`. **`logRequestFailure` is the only thing that ever sees the caught error**, never the record.
7. Evidence telemetry, counts only, in a swallowing `try` — lifted verbatim from `synthesis/route.ts:170-184`.
...
export function analysisStatus(i: Pick<StoredInterview, 'analysis' | 'synthesis'>):
  InterviewAnalysisStatus {
  return i.analysis ? i.analysis.status : i.synthesis ? 'complete' : 'pending';
}

/** True for pending, running and failed. */
export function isAwaitingAnalysis(i: …): boolean;
```
...
- **`pending`** — `Notice tone="neutral"`, eyebrow `Analysis pending`, body `This interview was saved. Its analysis has not run yet.`, plus a `Run analysis` primary button.
- **`running`** — `Notice tone="neutral"`, eyebrow `Analysis running`, body `An analysis started {relative time}. Give it a moment, then reload.` `Run analysis` is present but disabled until the lease elapses, after which the record reads `pending` on the next load anyway.
- **`failed`** — `Notice tone="error"`, eyebrow `Analysis failed`, a `Run analysis` button, and one sentence chosen by `failureKind` and naming nothing else:
...
The header stays `Status` (ruling 5). Slice O's `Conducted` and `Synthesized` model columns (O7.1) are unchanged and keep their `lg:table-cell` breakpoint; `Synthesized` reads `not recorded` for an un-analyzed interview, which is now a common and correct state rather than a legacy one.
...
**The batch action** sits on the Interviews tab above the register, only when the awaiting count is non-zero: `Analyze 3 pending`, a `variant="primary"` button, disabled while `operationPending` or a batch runs, labelled `Analyzing 2 of 3…` in flight.
...
```ts
  // validateStudyConfig, beside researcherContact's check:
  ...
  // validateStudyConfigForCreate:221 and validateStudyConfigUpdate:249 ONLY:
...
**Ruling 4 sets this order**, superseding the draft's "confirmation first, thank-you second":

```tsx
<Verbatim as="h1" className="text-[28px] font-normal leading-[36px] text-ink-900">Thank you</Verbatim>
...
<Rule className="mt-2" />   {/* …then the K2 <dl>, minus its contact row… */}
```

- **`Interview submitted` is retired as the heading; `Thank you` replaces it.** The safe-to-close sentence keeps its `role="status" aria-live="polite"`, its exact words, and its position as the first line under the heading — it is the fail-closed confirmation and it stays on DIRECTION §9's keep list, appearing **exactly once**. Everything that resolves the old heading by name must be updated in one pass: `research-workflow.spec.ts:66`, `Synthesis.register.test.tsx:79`, `Synthesis.completion.test.tsx:61,82`, and `Synthesis.lifecycle.test.tsx:50,73,79,134,180,185,201,208,225`. That is the largest single-string rename in the slice and it is mechanical; the sentence at `research-workflow.spec.ts:67` is unchanged.
...
`useStudyDraft.ts` gains `thankYouText` state and `setThankYouText` beside `researcherContact` (`:81`, `:189`, `:206`, `:239`), and one line in `buildConfig` beside `:225`: `...(thankYouText.trim() ? { thankYouText: thankYouText.trim() } : {})`. **Not** `|| defaultThankYouText(...)`, which is what the consent line one row above does — fact 23 is the difference. A blank field stays absent and the default is applied at render, so improving the default improves every study that never overrode it.

### P12.6 Tests

- **`tests/unit/thankYouText.test.ts`** — `defaultThankYouText` names and trims the study and contains no `[`; `THANK_YOU_TEMPLATE` contains one and `BRACKETED_PLACEHOLDER` matches it; the two are never equal.
- **`tests/unit/studyConfigValidation.test.ts`**, beside `:193-226` — create and update both refuse `[study name]` with `THANK_YOU_TEXT_PLACEHOLDER_ERROR`, both accept it absent, both accept 4 000 characters and refuse 4 001. **The guard that matters: plain `validateStudyConfig` accepts a config with no `thankYouText` *and* one carrying a bracketed value** — a study saved before this slice must still serve participants (fact 22).
- **`tests/unit/receiptFacts.test.ts` passes untouched, in full.** It exercises four pure functions over the transcript and the consent timestamp (`:9-95`); no fact changes, no signature changes, and the thank-you screen adds no derived value. If this file moves, the slice reached into the receipt's arithmetic, which it has no reason to.
- **`tests/unit/Synthesis.receipt.test.tsx`** — most of it stands and three cases move:
  - **Keeps passing, changed only by the ruling-4 heading rename:** `:70` (all three facts render and nothing guesses a value), `:123` (Elapsed omitted for a one-message transcript), `:138` (Consent accepted omitted with no placeholder text), `:156` (the safe-to-close sentence keeps `role="status"` and stays outside the fact block), `:173`/`:186`/`:199` (no fact block in the non-saved states — now two states, not three), `:213` (no `svg` and no `[role="note"]` in the saved state, which the thank-you sheet must not break).
  - **`:98-122` is rewritten.** "renders a Researcher contact row, not in mono (M9.5)" becomes "renders `Questions or concerns? Contact:` exactly once, below the thank-you text, with no `a[href^=\"mailto:\"]`, and no `Researcher contact` `<dl>` row". The M9.5 guarantee it protects — the contact is shown to the participant and never set in mono — survives in a better place; only the element changes.
  - **New cases:** no `thankYouText` renders the default and the document contains no `[`; a configured one renders verbatim including its line breaks; a blank `researcherContact` leaves the contact string absent with nothing in its place; `finalizing` and `save-failed` render no thank-you text at all.
...
- **Per-interview JSON already carries it.** `export/route.ts:120` and `InterviewDetail.tsx:97` serialize the whole record, so `synthesis: null` and the `analysis` object appear in both with no edit — the brief's requirement, met by doing nothing.
- **`summary.csv` gains one column**, appended last so `cells[6]` stays put (`tests/unit/api.export.csvFormulas.test.ts:106`), and after slice O's two if O has landed:
...
No backfill: no migration script, no `demoData.ts` edit (P2.19 — its three interviews carry syntheses and derive `complete`), no `makeStoredInterview` change (`tests/fixtures/models.ts:54` already defaults `synthesis: null`, so every fixture interview derives `pending`). The derivation runs in exactly two places — `analysisStatus()` for every read surface and the Lua claim path (P6.2) — so a legacy record cannot be re-analyzed by a claim that failed to notice its synthesis.
...
- **`tests/unit/kv.atomicPersistence.test.ts`** in full, especially `:105` (hosted key arity), `:126` (Finish ZADDs once, derives lock/count, deletes the guard last), `:150` (duplicate equivalence ignores request `Date.now`) and `:157` (Finish uses the frozen guard). If this file moves, the analysis writer reached into the persist protocol.
...
- **`tests/unit/Synthesis.lifecycle.test.tsx`** — every submission-identity, StrictMode-replay and stale-attempt case. The two analysis-failure cases (`:225-257`) go with the participant states they describe; **everything else in the file is the contract P3.1 promises not to break.**
- **`tests/unit/Synthesis.register.test.tsx`** — the three surviving participant states keep level-1 headings, zero icons, zero `role="note"` and zero `stone-*` classes; `:104-114` is deleted with `analysis-failed`, and `:79`'s heading name changes with ruling 4.
- **In full, untouched:** `api.save.evidenceRefs.test.ts` (apart from its `_receipt` bodies), `evidence.test.ts`, `evidence.aggregate.test.ts`, `api.aggregate.citations.test.ts`, `api.aggregate.revision.test.ts`, `kv.aggregatePersistence.test.ts`, `api.followup.provenance.test.ts`, `api.participant.canonicalContext.test.ts`, `wire.persist.test.ts`, `wire.authority.test.ts`, `wire.account.test.ts`, `Dashboard.idColumn.test.tsx`, `StudyDetail.register.test.tsx`, `StudyDetail.participantLinks.test.tsx`, `InterviewDetail.trace.test.tsx`, `Export.mode.test.tsx`, `Consent.contact.test.tsx`, `consentText.test.ts`, `DemoSimulation.*`.
...
2. **The `Interview submitted` → `Thank you` heading rename (ruling 4)** touches `Synthesis.register.test.tsx:79`, `Synthesis.completion.test.tsx:61,82`, every `Interview submitted` in `Synthesis.lifecycle.test.tsx`, `Synthesis.receipt.test.tsx`, and `research-workflow.spec.ts:66`. Mechanical, one pass, and no other string moves with it.
3. **`tests/unit/Synthesis.completion.test.tsx`** — `:63` becomes "shows participants a safe-to-close confirmation as soon as the save succeeds, with no synthesis call": assert `synthesizeInterview` is never called and `saveCompletedInterview` is called once with `synthesis: null`. `:77` (save failure, retry, then success) stands as written — storage retry is still the participant's.
4. **`tests/unit/api.synthesis.telemetry.test.ts`** — every case runs as `isAdmin`, the two refund cases are deleted, and one new case asserts a participant token gets 403 with no provider call. **`api.export.csvFormulas.test.ts`** gains one appended cell; every `cells[n]` index at `:88`, `:99`, `:106` is unchanged by construction.
...
- **`tests/unit/interviewAnalysis.idempotent.test.ts`** — the concurrency contract, against mocked kv primitives:
  - two `runInterviewAnalysis` calls in one `Promise.all` produce exactly **one** `synthesizeInterview` and exactly **one** `attachInterviewAnalysis`; the loser returns `busy`, and a run whose attach returns `stale` makes no second write attempt.
  - a run against an already-`complete` record makes no provider call at all.
  - a provider throw records `failureKind: 'provider'` and the thrown value appears nowhere in the arguments to `recordInterviewAnalysisFailure`; an oversized synthesis records `'too-large'` and never reaches `client.eval`.
...
- **`tests/e2e/research-workflow.spec.ts`** — the rewritten journey, and the one that answers the owner directly:
  1. Participant one finishes with `workflow.failNextSynthesis = true` set **before** the finish press. Assert the `Thank you` heading and `Your responses have been saved. It is now safe to close this tab.` **appear anyway**, that the default thank-you text is visible, and that no `Retry finalization` control exists on the page. Reload: the receipt still renders and no second save fires.
  2. Participant two finishes normally, with a storage fault at save time so `Retry save` is still exercised — the participant's one remaining retry.
  3. The researcher opens participant one's interview: the Analysis tab reads `Analysis pending` or `Analysis failed` and offers `Run analysis`. Press it; the reading renders and the footer names the **study's** model — `expect(interview.aiModel).toBe(studyModel)`, the concrete consequence of `feat/synthesis-uses-study-model`, which deleted the per-provider synthesis constants this spec previously imported here.
  4. `Analyze All Interviews` then succeeds and every existing aggregate-citation assertion (`:110-125`) passes unchanged.
...
**Superseded by this slice:** O7.2's reasoning is now false — "within one study the synthesis model is a per-provider constant" no longer holds, so StudyDetail's interview register gains the `Synthesized` column O declined, at the same `md:table-cell` breakpoint. O3.3's asymmetry stands (`Conducted by` is asked-for, `Synthesized by` is provider-reported) but both now name models the researcher chose. O13's `api.save.idempotent.test.ts` rewrite is absorbed into P16's larger rewrite of the same file; implement O's fingerprint keys and P's removals in one pass. O's open question 3 is untouched and still open.
...
```bash
npx vitest run \
  tests/unit/api.save.idempotent.test.ts tests/unit/api.save.noSynthesis.test.ts \
  ...
  tests/unit/kv.atomicPersistence.test.ts tests/unit/kv.analysisAttach.test.ts \
  ...
  tests/unit/analysisState.test.ts tests/unit/InterviewDetail.analysis.test.tsx \
...
- No new Redis key, prefix, delete-cascade arity change or fault cut. One new script, one new wire family. No new npm dependency; `jose` stays for `auth.ts`.
...
- **Renaming `synthesisReceipt.ts` to `provenance.ts`** once it holds only provenance; and **deleting the `AGGREGATE_*` receipt machinery**, already deferred by slice N. Housekeeping, its own diff.
- **Automatic retry of a failed analysis.** No backoff, no cron, no queue. A `failed` record waits for a researcher.
...
All five are settled by the Rulings below; the text above stands except where a ruling amends it (P3.1, P12.4 and P16 carry the amendments inline).
...
4. **Should the thank-you text lead the saved screen instead of following the confirmation?** The spec puts `Interview submitted` and `Your responses have been saved. It is now safe to close this tab.` first, because they are the fail-closed confirmation and both are on DIRECTION §9's keep list, with the researcher's words immediately beneath in the reading register. A "proper thank-you screen" might reasonably open with the thank-you and demote the system's sentence to a line under it. That is a keep-list change and four test suites resolve those strings, so it needs an explicit ruling rather than my judgement.

5. **Is `Status` the right header for the Dashboard's new analysis column, or should the existing `completed`/`in_progress` value keep it?** Today's column is dead — every stored interview is `completed`. Reusing the header avoids adding an eighth column to a register that already sheds four below 1024px. But `status` also names a real field on the record, and a future `in_progress` feature would want it back.

---

## Rulings (owner-delegated, 2026-09-05)

1. **Q1** — Allow analysis at an older study revision; record `analysis.studyRevision`; footer prints `analyzed at study rev N` when it differs. No refusal, no revert path.
2. **Q2** — `maxDuration = 120` stands. Project was created Dec 2025 on Vercel with Fluid compute, where 120 s is within the ceiling on every plan. If the platform cuts it short, the researcher's button is the recovery, as designed.
3. **Q3** — The participant receipt itself says nothing about analysis. The researcher-authored thank-you text (P12) is where any such sentence lives, at the researcher's discretion.
4. **Q4** — Reuse the `Status` header for analysis state. The record's `status` field stays as is; if `in_progress` ever becomes real, that is the moment to add a column.
5. Slice O is folded into this build: implement O's surviving parts (per P17) and P in ONE Codex pass on one branch, `feat/slice-p-save-first`, from `main` after `feat/synthesis-uses-study-model` merges (`f6bb49c`). StudyDetail's interview register gets both `Conducted` and `Synthesized` columns per P17.
6. Delete the two `_receipt`-related fault-cut entries only if `assertFaultCutsCovered()` demands it; otherwise leave `faultManifest.ts` untouched as P6.2 says.
7. Branch/commit: do not commit; leave the tree for review.

**Resolved (owner-delegated ruling, confirmed 2026-09-05).** The thank-you leads. Saved-state order: `<h1>` **`Thank you`** → the fail-closed line `Your responses have been saved. It is now safe to close this tab.` (verbatim, `role="status"`, first line under the heading, exactly once) → the researcher's thank-you sheet (default or override) → the contact line when present → the K2 receipt `<dl>`. `Interview submitted` is retired as the heading; update every test and the e2e that resolve it by name. P12.4 and P16 as written are correct — implement them.
