...
> **Status 2026-09-05 (evening).** K7 iPhone check passed: composer stayed above the keyboard on short and long transcripts — slice-K Open question 2 closed, no `visualViewport` fallback. Production incident (Gateway 403 → Gemini schema keywords `maxLength`/`minimum`/`minItems`/`maxItems` → participant retry lockout) closed by PRs #21–#23. Synthesis now uses the study's configured model (this PR). Next train: slice P (save first, analyze later + thank-you screen), absorbing slice O.
