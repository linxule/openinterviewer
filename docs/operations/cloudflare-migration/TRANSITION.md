...
4. **A deliberately late write is refused.**
   - On the old production URL, sign in as the researcher. Node sign-in reads no storage. Then create a study named `fence probe <date>`. The request must fail: on the current release it answers 503 (storage unavailable), not 2xx.
