...
| `--forget-provider-key <p[,p…]>` | `update` only: after a manual `wrangler secret delete`, remove the named non-default providers from `providerKeys` once their keys are observed gone; no upload, no deploy ([Provider keys](#provider-keys)). |
...
- Removing a key is a manual delete on Cloudflare followed by `--forget-provider-key`; the installer itself never deletes a secret:

  ```bash
  wrangler secret delete OPENAI_API_KEY --name oi-acme       # the Cloudflare-side act, by hand
  npm run setup:cloudflare -- update --install acme --env production --forget-provider-key openai --yes
  ```

  Between the two, `update` reports the missing key as drift and names the `--forget-provider-key` command. The operation refuses (exit 2) before any Cloudflare call a provider that is not recorded (a repeat run after success says when it was forgotten), the default provider (switch it with `--change-provider` first), a list that would leave no key, `all`, and any other operation in the same run; while any `pendingChange` exists it refuses with the command that finishes that change. It then runs the drift checks above, which do not require the named keys. If a named key is still bound, it refuses with the exact `wrangler secret delete <NAME> --name <worker>` command for each and changes nothing. Once every named key is observed gone, one receipt write removes the providers from `providerKeys` and appends one `secretEvents` entry (`kind` `forget-provider-key`, the key `names`, `uploaded` `[]`, `at`, `deploymentId` null), then `verify` runs. It uploads nothing, deploys nothing, needs no release artifact and records no `pendingChange`: the single atomic write is the whole effect, so an interrupted run either changed nothing or finished. Forget every key deleted by hand in one run (`--forget-provider-key claude,openai`): each run requires every other recorded key to be bound.
- A manual `wrangler secret delete` is expected to deploy a new Worker version, as `wrangler secret put` does (observed for `put`, not yet for `delete`), so after it the next key operation refuses until a plain `update` redeploys a checked release (the newest-deployment rule above). `--forget-provider-key` itself is not blocked by that rule, and its event is skipped when the rule looks for the deployment the previous key operation recorded. Studies set to a forgotten provider fail at their next call to it; the study editor offers only providers whose key is bound.
...
- with any option that asks for a change: `--provider`, `--provider-keys`, `--jurisdiction`, `--origin`, `--account-id`, `--import-target`, `--secrets-stdin`, `--operator-token-file`, `--reveal-operator-token`, `--change-provider`, `--add-provider-key`, `--rotate-provider-key`, `--forget-provider-key`, `--ai-transport`, `--change-ai-transport`, `--rotate-ai-gateway-token`, `--rotate-admin-password` or `--yes`;
...
- Provider keys (`tests/setup-cloudflare/provider-keys.test.mjs`):
  ...
  - `update --forget-provider-key` (`tests/setup-cloudflare/forget-provider-key.test.mjs`) after a simulated manual delete removes the providers from `providerKeys` and records one `secretEvents` entry with no deploy, upload or `wrangler secret delete`; a plain `update` then passes the drift check it failed before and `verify` passes; it refuses a still-bound key with the delete command and the receipt unchanged, an unrecorded or already forgotten provider, the default provider, the last key, `all`, a combined operation, a missing `--yes` and a pending change, all before any write; it exempts only the named keys from drift and forgets two deleted keys in one run; after it, the delete's deployment blocks the next key operation until a plain `update`, while a delete that made no deployment leaves the previous key operation's deployment accepted; `apply`, `resume` and `config` refuse the option;
