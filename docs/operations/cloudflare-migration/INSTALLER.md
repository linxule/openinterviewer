...
| `update` | Deploys the current checked artifact to an existing installation (`--yes`), runs one provider-key or Run-token operation without deploying ([Provider keys](#provider-keys)), or switches the provider or the AI transport and deploys ([AI transport](#ai-transport-cloudflare-ai-gateway)). |
...
| `--secrets-stdin` | Read `{"ADMIN_PASSWORD": "...", "<PROVIDER>_API_KEY": "...", ...}` from stdin: exactly the password, one key per `--provider-keys` entry and, with `cloudflare-gateway`, `CF_AI_GATEWAY_TOKEN` (update operations: exactly the named keys, or the Run token). Without it, a no-echo terminal prompt asks (the password twice). |
...
It never creates or deletes anything, and never rotates the generated secrets or the epoch. Provider keys change only on an explicit operation: `--change-provider --provider <name>` uploads the new provider's key alone when it is not yet bound, and the key operations below upload only the keys they name. `update` runs one operation at a time.
...
- **CI.** Regenerate `CLOUDFLARE_INSTALL_CONFIG` with `config` after a transport change ([below](#maintained-instance-ci-promotion)); key and token operations need no regeneration.
...
- with any option that asks for a change: `--provider`, `--provider-keys`, `--jurisdiction`, `--origin`, `--account-id`, `--import-target`, `--secrets-stdin`, `--operator-token-file`, `--reveal-operator-token`, `--change-provider`, `--add-provider-key`, `--rotate-provider-key`, `--ai-transport`, `--change-ai-transport`, `--rotate-ai-gateway-token` or `--yes`;
...
- **After `update --change-ai-transport`.** Likewise for `AI_TRANSPORT` and the two gateway vars: CI would otherwise deploy the previous transport again. Key and token operations change no var and need no regeneration.
