...
| `@cloudflare/vitest-plugin` | 1.2.3 (exact) | successor of `@cloudflare/vitest-pool-workers` (renamed 2026-08-19); pins the same wrangler 4.136.3 / miniflare / workerd as production. 0.22.0 was tried first and replaced because it pins workerd 1.20260815.1 and rejects compatibility dates after 2026-08-22 |
| `compatibility_date` | `2026-09-15` | supported by the single pinned workerd 1.20260921.1 |
...
Transaction/alarm probes (`cloudflare/probe/`), SQLite-backed DO in the same artifact as the OpenNext `fetch`. Rollback probes ran at compatibility date 2026-08-15 (and were independently reproduced by the platform research probe at 2026-09-20 in `createTestHarness` and the Vitest pool); both crash cuts were re-run at 2026-09-15 with identical results:
...
Local caveats: Durable Object jurisdictions throw `Jurisdiction restrictions are not implemented in workerd` in every local runtime, so jurisdiction is optional configuration applied only when set; SQLite PITR is not implemented locally.
