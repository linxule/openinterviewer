...
| Researcher AI budget (D15) | Every researcher-initiated provider call is admitted against `STANDALONE_RESEARCHER_AI_POLICY` (per researcher session and per workspace) before the provider: `admitResearcherAiRequest` checks every window, then charges every window, in one transaction in `budget_windows` under the `researcher-ai` gate (`open`, `draining`); keys are salted digests of `researcher-ai:` text, disjoint from participant keys. Analysis retries charge inside `acceptAnalysisRetry`, only when a new generation is allocated. Limited is 429 with `Retry-After`; an unusable budget is 503. No schema change. |
