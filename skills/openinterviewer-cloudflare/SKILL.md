...
- The project's own production installation (the maintained instance) is deployed with this installer from the owner's workstation, like any other installation: `update --install <install> --env production --state-dir <dir> --yes` from a clean checkout of the release commit after `npm ci` and `npm run check:cloudflare` ([section 2](#2-prepare-the-checked-artifact)). Its receipt is in a state directory outside the checkout: use the `--state-dir` the owner names. Deploy it, or run any other `update` operation on it, only on the owner's explicit go-ahead for that run. The `promote-cloudflare` CI job is not its deployment owner and stays unconfigured: do not dispatch it or configure it for this instance ("Maintained instance" in INSTALLER.md).
...
- Install or update? For an update, the installation name and environment must match an existing receipt in `cloudflare/installations/<install>-<env>/receipt.json` (or `<dir>/<install>-<env>/receipt.json` with `--state-dir <dir>`).
...
- For an update or rollback: is this the project's maintained instance? If so, get the owner's explicit go-ahead for this run and the `--state-dir` that holds its receipt (Hard rules).
- Fresh install only: the default AI provider (`gemini`, `claude`, `openai` or `openrouter`) and which other providers' keys to bind now (`--provider-keys`, default only the default provider's; more can be added later without a redeploy), the Durable Object jurisdiction (`eu` recommended, `fedramp`, or `none`; it cannot be changed later without a migration), whether a final HTTPS origin already exists (otherwise the `workers.dev` origin is discovered from the first deploy), and the AI transport (below). A custom domain is attached by the person in the dashboard after the first deploy; the installer never changes DNS or routes.
- AI transport: `direct` (default) or `cloudflare-gateway`, which sends every provider request through the installation's own Cloudflare AI Gateway (RT-11) with logging, caching and retries off. Choose the gateway only when the person asks for it. It needs two credentials the person creates in the Cloudflare dashboard (a management token with AI Gateway Read and Edit, and an AI Gateway Run token), changes the participant consent notice (the person should confirm the new wording is approved for their studies), and rests on live Cloudflare behaviour not yet observed (remote gates in `docs/operations/cloudflare-migration/evidence/DEVIATIONS.md`, RT-11). Until gate S1 is confirmed, tell the person that Gemini studies fail on the gateway.
- For an update, which one operation (one per run): deploy the current release; switch the default provider (`--change-provider`); add or replace provider keys (`--add-provider-key`, `--rotate-provider-key`); switch the transport (`--change-ai-transport`); replace the Run token (`--rotate-ai-gateway-token`); replace the administrator password (`--rotate-admin-password`); record a key the person deleted by hand (`--forget-provider-key`; the installer never deletes a secret: point the person to RUNBOOK, provider keys).
...
```bash
npm run setup:cloudflare -- plan --install <name> --env <env> [--provider <p>] [--jurisdiction <j>] [--origin https://…] [--ai-transport <t>]
```
...
Update refuses configuration drift, identity or jurisdiction changes and an unready artifact before deploying. Switching providers needs `--change-provider --provider <p>`, the new key on stdin (`--secrets-stdin`) when it is not already bound, and the person's explicit request. An interrupted provider change stays recorded as pending in the receipt; rerun the same `update --provider <p> --change-provider` to finish it (other `update`, `resume`, `apply` and `config` runs refuse until it is finished). To bind another provider's key, `update --add-provider-key <p[,p]> --secrets-stdin --yes` with only those keys on stdin; to replace one, `update --rotate-provider-key <p> --secrets-stdin --yes`. Both are one secret upload with no deploy, need the person's explicit request, and finish the same way when rerun after an interruption. To remove a non-default key, the person runs `wrangler secret delete <NAME> --name <worker>` (never do this for them without their explicit request), then `update --forget-provider-key <p[,p]> --yes` records it: no upload, no deploy; it refuses while the key is still bound and prints the delete command, and refuses the default provider. After the delete, a plain `update` must run before the next key operation. The Cloudflare AI Gateway transport (RT-11) is an explicit choice of the person: `apply --ai-transport cloudflare-gateway` on a fresh install, or `update --change-ai-transport --ai-transport <cloudflare-gateway|direct> --yes` later. It needs `CF_AI_GATEWAY_ADMIN_TOKEN` in the installer's environment only (for example `op run --env-file <file outside the checkout> -- npm run setup:cloudflare -- …`, run by the person or with their explicit authorization; never pass it on stdin, in argv or in a file in the checkout) and the Run token as `CF_AI_GATEWAY_TOKEN` on stdin when it is not yet bound. Before switching, tell the person what the installer prints about consent (participants consented under one transport may need to reopen their link; approved studies may need an ethics amendment for the gateway notice) and that the RUNBOOK drains the workspace first. A gateway refusal (collision, settings drift, a failed probe) is a stop-and-ask: never change or delete a gateway. `update --rotate-ai-gateway-token --secrets-stdin --yes` replaces the Run token. `update --rotate-admin-password --secrets-stdin --yes` with `{"ADMIN_PASSWORD": "…"}` replaces the administrator password (one upload, no deploy, only on the person's explicit request); tell the person that researcher sessions signed in before it stay valid until they expire (up to 7 days) and point to RUNBOOK, administrator password, after a leak. Exit 3 means the new version is deployed but the workspace is held by an operator (`draining`, `frozen` or `recovery`); report it and run `verify` after they reopen it.
...
- on the gateway transport: whether `verify` read the gateway settings and stored-log count (only with the management token), and that pass-through to each provider is not verified by `verify` (RT-11 remote gates in DEVIATIONS.md).

For a deploy made outside the installer, such as a CI promotion on an installation that chose CI ownership, `npm run setup:cloudflare -- verify --config <installation wrangler.jsonc> [--wait-seconds <n>]` checks the config and the readiness of its `APP_BASE_URL` without a receipt. It exits 0 when ready, 1 when not ready, 2 when refused and 3 when held.

An installation deployed by the optional `promote-cloudflare` CI job (not the maintained instance; "Optional: CI-owned deployment" in INSTALLER.md) needs its installation config stored as the GitHub variable `CLOUDFLARE_INSTALL_CONFIG`. Produce it with:

```bash
npm run setup:cloudflare -- config --install <name> --env production [--json]
```

It rewrites `cloudflare/installations/<install>-<env>/wrangler.jsonc` from `receipt.json` and the current `wrangler.jsonc`, and prints that path as its last line (`path` with `--json`). It reads only the receipt and the template and writes only that git-ignored file: no wrangler, deploy, git or network call, and nothing in the person's account. It refuses (exit 2) without a receipt, before the installation cleared its bootstrap (`installation <name> (<env>) is not complete: WORKSPACE_BOOTSTRAP has not been cleared`), with any option that asks for a change (`--provider`, `--yes`, `--secrets-stdin` and the like), and while any installer change is pending. Give the person the path; the file holds names, vars and queue names only. Run it again after `update --change-provider` or `update --change-ai-transport` (key and token operations need no rerun), and after any release that changes `wrangler.jsonc` outside the installation-owned fields (`name`, var values, `routes`, `workers_dev`, `account_id`, queue names), for example the compatibility date or flags, Durable Object migrations or bindings, var names or queue settings.
