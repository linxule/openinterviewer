...
- For an update, which one operation (one per run): deploy the current release; switch the default provider (`--change-provider`); add or replace provider keys (`--add-provider-key`, `--rotate-provider-key`); switch the transport (`--change-ai-transport`); replace the Run token (`--rotate-ai-gateway-token`); replace the administrator password (`--rotate-admin-password`). Removing a key has no installer operation: point the person to RUNBOOK, provider keys.
